...
def classify_targets(bug_info, targ_list, bug_sigs):
    ...
    for targ in targ_list:
        ...
        if (BD_sig, True) in bug_info[targ]:
            BD_list.append(targ)            
    return (ME_list, BD_list, IB_list)
...
def main():
    ...
    bug_sigs = get_tool_sigs(sig_set, [ME, IB, BD])
    ME_sig, IB_sig, BD_sig = bug_sigs

    bug_info = init_b5_bug_info(ME_sig, IB_sig, BD_sig)
    ...
    ME_list,BD_list, IB_list = classify_targets(bug_info, targ_list, bug_sigs)
