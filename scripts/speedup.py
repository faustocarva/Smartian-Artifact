...
def print_speedup_summary(speedups: Dict[str, Dict[str, float]], 
    ...
    if all_valid_speedups:
        ...
        print(f"New method faster: {better_count} contracts ({better_count/len(all_valid_speedups)*100:.1f}%)")
        print(f"Baseline faster: {worse_count} contracts ({worse_count/len(all_valid_speedups)*100:.1f}%)")
        if same_count:
            print(f"Equal performance: {same_count} contracts ({same_count/len(all_valid_speedups)*100:.1f}%)")
        print()
